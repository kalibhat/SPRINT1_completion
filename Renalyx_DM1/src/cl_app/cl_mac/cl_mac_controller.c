				...
				{			
				...
				
					{
						...
						case EVT_CONSOLE_COMMAND_RINSE_START:
							...
							Cl_MacRetval = Cl_Standby_Controller(Cl_MacEvent);
							ASSERT_ERROR(Cl_MacRetval);
// Make chages to set hotrinse flag
							if(Cl_MacRetval == CL_OK)
							{
...
Cl_ReturnCodeType 	Cl_MacInit(void) 
{
		...
		Cl_MacEventQueue4.Cl_MacNewEventcountDummy16 = 0;
		Cl_MacEventQueue4.Cl_MacNewEventcountDummy8 = 0;
...
Cl_ReturnCodeType	Cl_MacCreate_Macevent(void)
{
	...
	switch(queue_sequence_counter){
		...
		case 3: // alarm
	/*
		if(Cl_MacEventQueueAlarm.Cl_MacNewEventcount > 0){
				Cl_MacNewEvent = true;
				Cl_MacEvent = Cl_MacEventQueueAlarm.Cl_MacEvent[0];
				Cl_MacEventQueueAlarm.Cl_MacNewEventcount--;
			//	Cl_MacEventQueue4.Cl_MacNewEventcountDummy16--;
			//	Cl_MacEventQueue4.Cl_MacNewEventcountDummy8--;
				
			//	if(
				
				for (tempcount =0 ;tempcount < Cl_MacEventQueueAlarm.Cl_MacNewEventcount;tempcount++ )
				{
					
					if(tempcount < MAC_EVENT_COUNT_MAX)
					{
						Cl_MacEventQueueAlarm.Cl_MacEvent[tempcount] = Cl_MacEventQueueAlarm.Cl_MacEvent[tempcount+1];
					...
					else
					{
						if(tempcount >= MAC_EVENT_COUNT_MAX)
						{
							Cl_MacEventQueueAlarm.Cl_MacEvent[tempcount] = EVT_NULL;
						}
						// handle error, raise alarm. system stops
						break; //handle exception
					}
