...
Cl_Uint8Type Seccounter_Propo=0;
 ...
 */
  ...
  DisinfAlarmsType Cl_DisinfAlarmTable[CL_DISINF_ALRM_MAX] =
  {
	  ...
	  {ACID_IN_OPEN,CL_ALARM_ALARM,false,false,false},
	  {BICARB_IN_OPEN,CL_ALARM_ALARM,false,false,false},
...
// 				
		...
		//			break;
				...
				{ 
					...
					// check for respective probes
					 ...
					 if (flag_hotcitric == 1 || flag_citric == 1)
					 {
						...
						Cl_AlarmConfigureAlarmType(ACID_IN_OPEN,LOGIC_LOW,0,0,0);
						Cl_AlarmConfigureAlarmType(BICARB_IN_CLOSED,LOGIC_HIGH,0,0,0);
						 Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_OPEN,false );
						 Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_CLOSED,true );
						 Cl_disinfretcode =  Cl_AlarmActivateAlarms(ACID_IN_OPEN,true );
						 Cl_disinfretcode =  Cl_AlarmActivateAlarms(ACID_IN_CLOSED,false );
					 }
					 else if (flag_na == 1)
					 {
						 
						Cl_AlarmConfigureAlarmType(BICARB_IN_OPEN,LOGIC_LOW,0,0,0);
						Cl_AlarmConfigureAlarmType(ACID_IN_CLOSED,LOGIC_HIGH,0,0,0);	
					 
					 Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_OPEN,true );
					 Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_CLOSED,false );
					 Cl_disinfretcode =  Cl_AlarmActivateAlarms(ACID_IN_OPEN,false );
					 Cl_disinfretcode =  Cl_AlarmActivateAlarms(ACID_IN_CLOSED,true );
			...
			//		  Cl_disinfretcode = Cl_SendDatatoconsole(command,&data,0);
					...
					sv_cntrl_activatepump(DCMOTOR1);              // Gopal: added to run dc motors to run during intake
					sv_cntrl_activatepump(DCMOTOR2);
					 cl_disinfstate = CL_DISINF_STATE_DISINF_OPENFILL;
			...
			case CL_DISINF_STATE_DISINF_INTAKE:
					...
					//			UpdateDisinfMinuteTick();
							...
							if (flag_hotcitric == 1 )
							{
								...
								//	Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_OPEN,false );  // Gopal:  Added on 16112017
								//	Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_CLOSED,false );  // Gopal:  Added on 16112017
									Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_OPEN,false );   // Gopal: Added on 16112017 
								Cl_disinfretcode =  Cl_AlarmActivateAlarms(ACID_IN_OPEN,false );   // Gopal: Added on 16112017 
								Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_CLOSED,false );   // Gopal: Added on 16112017
								Cl_disinfretcode =  Cl_AlarmActivateAlarms(ACID_IN_CLOSED,false );  
						...
						//			cl_disinfstate = CL_DISINF_STATE_DISINF_INTAKE_COMPLETED;
							...
							if (flag_citric == 1 )
							{
								...
								Cl_disinfretcode =  Cl_AlarmActivateAlarms(ACID_IN_OPEN,false );   // Gopal: Added on 16112017 
								Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_CLOSED,false );   // Gopal: Added on 16112017
								Cl_disinfretcode =  Cl_AlarmActivateAlarms(ACID_IN_CLOSED,false );    // Gopal:  Added on 16112017
									sv_prop_stop_citricdisinfect_intake();
									Cl_disinfretcode =  sv_cntrl_deactivatepump(DCMOTOR2);
									Cl_disinfretcode =  sv_cntrl_deactivatepump(DCMOTOR1);
									Cl_bc_controller(BC_EVENT_STOP);
			...
			case CL_DISINF_STATE_DISINF_INTAKE_COMPLETED:
						...
						switch (cl_disinfevent)
						{
							...
							break;
								...
								case EVT_RESUME_DISINF_STAGE:
								Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_OPEN,true );   // Gopal: Added on 16112017 
								Cl_disinfretcode =  Cl_AlarmActivateAlarms(ACID_IN_OPEN,true );   // Gopal: Added on 16112017 
								Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_CLOSED,false );   // Gopal: Added on 16112017
								Cl_disinfretcode =  Cl_AlarmActivateAlarms(ACID_IN_CLOSED,false );   // Gopal: Added on 16112017
							...
							//	Cl_disinfretcode =  Cl_AlarmActivateAlarms(BICARB_IN_CLOSED,true );   // Gopal: Added on 16112017
							//	Cl_disinfretcode =  Cl_AlarmActivateAlarms(ACID_IN_CLOSED,true );   // Gopal: Added on 16112017
								if (flag_hotcitric == 1)
 								{
 								hotrinse_flag = 1;
 								}
...
// 								}
							...
							//	sv_prop_startmixing();     // manav03 commented
								sv_prop_startpropeo_aligning(); // Added for alignment before open fill restarting
...
// // 							}
							...
							if(Cl_disinfMinutescounter > CL_DISINF_TIMEOUT )							{
								...
								sv_cntrl_poweroffheater();
//								sv_prop_stopmixing();
								sv_prop_stopopenfill();			// Gopal: changed to stop open fill
...
//	sv_prop_stopmixing();
	sv_prop_stopopenfill();					// Gopal: changed from stop mixing to stop open fill since  on stop n re start, mixing pumps runs slowly
...
Cl_disinfretcode =  Cl_AlarmActivateAlarms(FLOW_NO_FLOW,false );					
		Cl_disinfretcode =  Cl_AlarmActivateAlarms(FLOW_LOW_FLOWRATE,false );
		Cl_disinfretcode =  Cl_AlarmActivateAlarms(FLOW_HIGH_FLOWRATE,false );
...
Cl_ReturnCodeType Cl_disinf_ResetAlertsforReassertion(void )
{
	...
	Cl_DisinfAlarmTable[CL_DISINF_ALARM_ACIDIN_CLOSED].IsRaised = false;
	Cl_DisinfAlarmTable[CL_DISINF_ALARM_BICARBIN_CLOSED].IsRaised = false;
	...
	Cl_DisinfAlarmTable[CL_DISINF_ALARM_TEMP1_HIGH_THRESHOLD].IsRaised = false;
	...
	Cl_DisinfAlarmTable[CL_DISINF_ALARM_FLOW_HIGH_FLOWRATE].IsRaised = false;
	...
	Cl_DisinfAlarmTable[CL_DISINF_ALARM_CONSOLE_SYNC_LOST].IsRaised = false;
