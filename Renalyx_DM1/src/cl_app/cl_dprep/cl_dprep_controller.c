...
DprepAlarmsType Cl_DprepAlarmTable[CL_DPREP_ALRM_MAX] =
{
	...
	{PREP_FAILED_COND_HIGH,CL_ALARM_ALARM,false,false,false},
	{PREP_FAILED_TEMP_HIGH,CL_ALARM_ALARM,false,false,false},
	{PREP_FAILED_COND_LOW,CL_ALARM_ALARM,false,false,false},
	{PREP_FAILED_TEMP_LOW,CL_ALARM_ALARM,false,false,false},
	{CONSOLE_SYNC_LOST,CL_ALARM_ALARM,false,false,false}  	          // manav06 added
};
...
// 			 cl_dprep_primecontroller(CL_DPREP_PRIME_PRIME_STOP_DIALYSER_PRIMING,0);
		...
		if(Cl_Check_Temp_Cond_Stability() == CL_OK || Cl_Check_Temp_Cond_Stability() == CL_WAIT ){
...
Cl_ReturnCodeType  CL_DrepAlarmActon(Cl_NewAlarmIdType cl_dprepalarmid)
{
	...
	switch(cl_dprepalarmid)
	{
		...
		break;
			...
			case PREP_FAILED_COND_HIGH:
			case PREP_FAILED_TEMP_HIGH:
			case PREP_FAILED_COND_LOW:
			case PREP_FAILED_TEMP_LOW:
...
Cl_ReturnCodeType  Cl_dprepSendAlarm(Cl_NewAlarmIdType cl_dprepalarmId )
{
	...
	Cl_ReturnCodeType Cl_Dprep_CheckforfillingCompletion(void)
	{
		...
		if(Cl_Dprep_filling_TotalMinutescounter > CL_DPREP_FILLING_TIMEOUT_MIN )
		{
			...
			// check for the reason , whether cond or temperature
			// raise alarm
			Cl_SysStat_GetSensor_Status_Query(SENSOR_TEMP3STATUS,&cl_temp3);
			
			if((cl_temp3/10) > Cl_alarmThresholdTable.temp3_high_threshold){
				
				 Cl_Alarm_TriggerAlarm(PREP_FAILED_TEMP_HIGH, 1);
			}
			
			else if((cl_temp3/10) < Cl_alarmThresholdTable.temp3_low_threshold){
				
				Cl_Alarm_TriggerAlarm(PREP_FAILED_TEMP_LOW, 1);
			}
			
			Cl_SysStat_GetSensor_Status_Query(SENSOR_COND_STATUS,&cl_cond);
			
			if((cl_cond/10) > Cl_alarmThresholdTable.cond_high_threshold){
				
				 Cl_Alarm_TriggerAlarm(PREP_FAILED_COND_HIGH, 1);
				
			}
			else if((cl_cond/10) < Cl_alarmThresholdTable.cond_low_threshold){
				
				 Cl_Alarm_TriggerAlarm(PREP_FAILED_COND_LOW, 1);
			}
			
			
		}
		else if(Cl_Check_Temp_Cond_Stability() == CL_OK){
...
Cl_DprepAlarmTable[CL_DPREP_ALARM_PREPFAIL_COND_HIGH].IsRaised = false;
Cl_AlarmResetAlarm(PREP_FAILED_COND_HIGH);

Cl_DprepAlarmTable[CL_DPREP_ALARM_PREPFAIL_TEMP_HIGH].IsRaised = false;
Cl_AlarmResetAlarm(PREP_FAILED_TEMP_HIGH);

Cl_DprepAlarmTable[CL_DPREP_ALARM_PREPFAIL_COND_LOW].IsRaised = false;
Cl_AlarmResetAlarm(PREP_FAILED_COND_LOW);

Cl_DprepAlarmTable[CL_DPREP_ALARM_PREPFAIL_TEMP_LOW].IsRaised = false;
Cl_AlarmResetAlarm(PREP_FAILED_TEMP_LOW);

Cl_DprepAlarmTable[CL_DPREP_ALARM_CONSOLE_SYNC_LOST].IsRaised = false;
			...
			//	 #endif		 
				 if(Cl_dprepreaduinessflag == false)
				 {
					 return CL_REJECTED;
							...
							Cl_dprepretcode =  Cl_AlarmConfigureAlarmType(TEMP1_HIGH_THRESHOLD,ABOVE,400,0,0);  // Gopal: change this to simulate high inlet temp alarm
...
// 							Cl_dprepretcode =  Cl_AlarmActivateAlarms(TEMP3_HIGH_THRESHOLD,true );
							 ...
							 Cl_dprepretcode =  Cl_AlarmActivateAlarms(PREP_FAILED_TEMP_HIGH,true );
							 Cl_dprepretcode =  Cl_AlarmActivateAlarms(PREP_FAILED_COND_HIGH,true );
							 Cl_dprepretcode =  Cl_AlarmActivateAlarms(PREP_FAILED_TEMP_LOW,true );
							 Cl_dprepretcode =  Cl_AlarmActivateAlarms(PREP_FAILED_COND_LOW,true ); 
						...
						//	cl_dprepbypassstate = CL_DPREP_BYPASS_OFF;
							...
							switch (temp)
							{
								...
								case 500:
								Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTTEXT,"500",3);
								sv_cntrl_setpumpspeed(DCMOTOR2,1000);    // 1000
								sv_cntrl_setpumpspeed(DCMOTOR1,1050);   //1000
								potvalue = (450 * 1024)/10000;          // 600
								sv_cs_setpotvalue(potvalue);
								...
								default:
								sv_cntrl_setpumpspeed(DCMOTOR2,1000);
								sv_cntrl_setpumpspeed(DCMOTOR1,1050);   //960
								potvalue = (450 * 1024)/10000;
								sv_cs_setpotvalue(potvalue);
...
// 		
		...
		//	cl_dprepbypassstate = CL_DPREP_BYPASS_OFF;
			...
			switch (temp)
			{
				...
				Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTTEXT,"500",3);
				sv_cntrl_setpumpspeed(DCMOTOR2,1000);
				sv_cntrl_setpumpspeed(DCMOTOR1,1050);                           // 420
				potvalue = (450 * 1024)/10000;
				sv_cs_setpotvalue(potvalue);
				break;
			}
		...
		Cl_Dprep_DialyserPrimeTimeInfo();
...
Cl_ReturnCodeType Cl_Dprep_DialyserPrimeTimeInfo(void)
{
	
	Cl_ReturnCodeType  Cl_dprep_primeretcode = CL_ERROR;
	Cl_ConsoleTxCommandtype command = CON_TX_COMMAND_COMMAND_MAX;
	Cl_Uint8Type data[2] = {0,0};
	...
	Cl_dprep_primeretcode = Cl_SendDatatoconsole(command,&data,2);
...
Cl_ReturnCodeType Cl_Check_Temp_Cond_Stability(){
	...
	// manav02 changed
	Cl_Int16Type cl_cond,cl_temp3;
	bool flag_temp = false , flag_cond = false;
//	Cl_Heater_GetHeaterStableStatus(&dprep_heater_stable_status);
	
		Cl_SysStat_GetSensor_Status_Query(SENSOR_TEMP3STATUS,&cl_temp3);
		
		if(((cl_temp3/10) < Cl_alarmThresholdTable.temp3_high_threshold) && ((cl_temp3/10) > Cl_alarmThresholdTable.temp3_low_threshold)){
		...
		if(((cl_cond/10) < Cl_alarmThresholdTable.cond_high_threshold) && ((cl_cond/10) >= Cl_alarmThresholdTable.cond_low_threshold)){
		
			flag_cond = true;	
					
		}
		if(flag_temp && flag_cond){
...
Cl_ReturnCodeType Cl_deactivate_prep_alarms(){
	...
	Cl_ReturnCodeType Cl_dprepretcode = CL_OK;
			...
			Cl_dprepretcode =  Cl_AlarmActivateAlarms(BP_STOPPED,false );
			Cl_dprepretcode =  Cl_AlarmActivateAlarms(PREP_FAILED_TEMP_HIGH,false );
			Cl_dprepretcode =  Cl_AlarmActivateAlarms(PREP_FAILED_COND_HIGH,false );
	Cl_dprepretcode =  Cl_AlarmActivateAlarms(PREP_FAILED_TEMP_LOW,false );
	Cl_dprepretcode =  Cl_AlarmActivateAlarms(PREP_FAILED_COND_LOW,false );
