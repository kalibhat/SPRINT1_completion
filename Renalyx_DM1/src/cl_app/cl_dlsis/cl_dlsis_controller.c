...
extern Cl_Uint16Type extuf_period;
...
{HEPARIN_END_DETECTION , CL_ALARM_ALARM,false,false,false },
{UFP_OVERRUN , CL_ALARM_ALARM,false,false,false},
{UFP_UNDERRUN , CL_ALARM_ALARM,false,false,false},
...
Cl_dlsisretcode =  cl_dlsis_translatemacevent( Cl_MacDlsisEvent, &cl_dlsis_event);
	...
	switch(cl_dlsis_event)
	{
		...
		
		{
			...
			else if(Cl_ConsoleRxMsg.data.byte[0] == 0)
			{
					...
					cl_hep_controller(CL_HEP_EVENT_START_NORMAL_DELIVERY,Treatdata[ID_heprate]);
									...
									Cl_AlarmActivateAlarms(HEPARIN_END_DETECTION ,true);
			}
		...
		case EVENT_DLSIS_HEP_OFF:
									Cl_AlarmActivateAlarms(HP_OVERRUN,false);
									Cl_AlarmActivateAlarms(HP_UNDERRUN,false);
									Cl_AlarmActivateAlarms(HEPARIN_END_DETECTION ,false);
...
// 			break;
			...
			if(bpflagCounter > 120){
				...
				cl_Dlsis_checkforHeparinCompletion();
...
Cl_ReturnCodeType cl_Dlsis_checkforHeparinCompletion(void){
		 ...
		 if((DlsisTime.Cl_dlsisTotalMinutescounter >= (Treatdata[ID_treattime] - Treatdata[ID_hepstoptime]) ) && hepStopFlag )
		 {
...
Cl_ReturnCodeType Cl_Dlsis_UpdateAlarmTable(ClDlsisAlarmIdType * Cl_DlsisAlarmId )
{
	Cl_ReturnCodeType 	Cl_dlsisretcode = CL_OK;
...
// 	  Cl_dlsisretcode =  Cl_AlarmActivateAlarms(FLOW_NO_FLOW,true );              // Gopal: Added low inlet and high inlet RO alarm in bypass
// 	  Cl_dlsisretcode =  Cl_AlarmActivateAlarms(FLOW_LOW_FLOWRATE,true );         // Gopal: Added low inlet and high inlet RO alarm in bypass
...
Cl_ReturnCodeType Cl_Dlsis_ResetAlertsforReassertion(void )
{
	...
	Cl_DlsisAlarmTable[CL_DLSIS_ALARM_COND_STATUS_LOW].IsRaised = false;
	Cl_DlsisAlarmTable[CL_DLSIS_ALARM_COND_STATUS_HIGH].IsRaised = false;
	Cl_AlarmResetAlarm(COND_STATUS_LOW);
	Cl_AlarmResetAlarm(COND_STATUS_HIGH);
	...
	Cl_DlsisAlarmTable[CL_Dlsis_ALARM_HEPARINEND].IsRaised = false;
	Cl_AlarmResetAlarm(HEPARIN_END_DETECTION);
	
	Cl_DlsisAlarmTable[CL_Dlsis_ALARM_UFUNDERRUN].IsRaised = false;
	Cl_AlarmResetAlarm(UFP_UNDERRUN);
	
	
	Cl_DlsisAlarmTable[CL_Dlsis_ALARM_UFOVERRUN].IsRaised = false;
...
//	Cl_AlarmResetAlarm(HOLDER2STATUS_CLOSED);
	...
	Cl_DlsisAlarmTable[CL_Dlsis_ALARM_CONSOLE_SYNC_LOST].IsRaised = false;
...
Cl_ReturnCodeType	Cl_Dlsis_ResumeDialysis(void)
{
		...
		Cl_ConsoleTxCommandtype command = CON_TX_COMMAND_COMMAND_MAX;
		Cl_Uint8Type data = 0;
									Cl_dlsisretcode =  Cl_AlarmConfigureAlarmType(FLOW_LOW_FLOWRATE,0,0,0,0);
									Cl_dlsisretcode =  Cl_AlarmConfigureAlarmType(FLOW_NO_FLOW,0,0,0,0);
									Cl_dlsisretcode =  Cl_AlarmConfigureAlarmType(FLOW_HIGH_FLOWRATE,0,1500,0,0);
								...
								//	Cl_dlsisretcode =  Cl_AlarmActivateAlarms(COND_DAC_HIGH,true );
									...
									Cl_dlsisretcode =  Cl_AlarmActivateAlarms(APTSTATUS_LOW,true );
									Cl_dlsisretcode =  Cl_AlarmActivateAlarms(VPTSTATUS_LOW,true);
									Cl_dlsisretcode =  Cl_AlarmActivateAlarms( BLD_EVENT,true );    // commented manav 06
									Cl_dlsisretcode =  Cl_AlarmActivateAlarms(PS2_HIGH_THRESHOLD,true );
									Cl_dlsisretcode =  Cl_AlarmActivateAlarms(PS2_LOW_THRESHOLD,true );
									Cl_dlsisretcode =  Cl_AlarmActivateAlarms(PS1_HIGH_THRESHOLD,true );
									Cl_dlsisretcode =  Cl_AlarmActivateAlarms(PS1_LOW_THRESHOLD,true );
