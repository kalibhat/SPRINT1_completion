...
Cl_Uint8Type new_hotrinse_flag = 0;
...
int cl_propstartcnt = 0;
extern int RinseCScount;
extern Cl_BoolType IntakeReadyFlag;
extern Cl_Uint8Type flag_hotcitric ;
...
{	
	...
	{ACID_OVERRUN,CL_ALARM_ALARM,false,false,false},
	{ACID_UNDERRUN,CL_ALARM_ALARM,false,false,false},
	{BICARB_OVERRUN,CL_ALARM_ALARM,false,false,false},
	{BICARB_UNDERRUN,CL_ALARM_ALARM,false,false,false},
...
//	{FPCURRENTSTATUS,CL_ALARM_ALERT,false,false,false},                              // added 17.03.17
	{CONSOLE_SYNC_LOST,CL_ALARM_ALARM,false,false,false},
	{UFP_OVERRUN,CL_ALARM_ALARM,false,false,false}, 
	{UFP_UNDERRUN,CL_ALARM_ALARM,false,false,false}                         // added on 18.03.17
...
//						cl_Datastreamtype cl_temp  ;
						...
						Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP2_HIGH_THRESHOLD,0,450,150,0);
						...
						Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP1_HIGH_THRESHOLD,0,370,150,0);
						Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP2_HIGH_THRESHOLD,0,450,150,0);
					//	Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP3_HIGH_THRESHOLD,0,400,150,0);
						}
						else if (cl_local.bytearray[0] == 2)
						{
							hotrinse_flag = 1;
						
							Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP1_HIGH_THRESHOLD,0,900,150,0);
							Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP2_HIGH_THRESHOLD,0,900,150,0);
						//	Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP3_HIGH_THRESHOLD,0,900,150,0);
						}
...
// 						break;
				...
				case EVENT_START_PRE_DISINF_RINSE:
					...
					if(flag_hotcitric){
						...
						Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP1_HIGH_THRESHOLD,0,900,150,0); // Gopal: TS1 threshold during hot citric 90 degress
						Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP2_HIGH_THRESHOLD,0,900,150,0); // Gopal: TS2 threshold during hot citric 90 degress
					}
					else{
						Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP1_HIGH_THRESHOLD,0,370,150,0);  // Gopal: Set to 250 to simulate high inlet temp alarm simulation
						Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP2_HIGH_THRESHOLD,0,450,150,0);
					}
...
// 							}
						...
						if(IntakeReadyFlag && (RinseCScount == 2))
						{
					...
					//	UpdateHeaterControls();
						...
						//	sv_cntrl_deactivate_valve(VALVE_ID8);
							...
							Cl_AlarmActivateAlarms(ACID_OVERRUN,false);
							Cl_AlarmActivateAlarms(ACID_UNDERRUN,true);
							Cl_AlarmActivateAlarms(BICARB_OVERRUN,false);
							Cl_AlarmActivateAlarms(BICARB_UNDERRUN,true);
						...
						/*	if(hotrinse_flag ==1)
							{
								if(Cl_RinseMinutescounter == (CL_BC_OPENFILL_TIMEOUT - 1))
								{
								new_hotrinse_flag =	hotrinse_flag ;	
							...
							*/
							...
							// manav07 added should check for hotrinse flag
							if(hotrinse_flag){
							IntakeReadyFlag = true;
							}
								Cl_AlarmActivateAlarms(ACID_OVERRUN,false);
								Cl_AlarmActivateAlarms(ACID_UNDERRUN,false);
								Cl_AlarmActivateAlarms(BICARB_OVERRUN,false);
								Cl_AlarmActivateAlarms(BICARB_UNDERRUN,false);
						...
						//	Cl_RinseTotalMinutescounter++;
							...
							{
								if((cl_rinsestate == CL_RINSE_STATE_RINSING) || (cl_rinsestate == CL_RINSE_STATE_POST_DISINFECT_RINSING)){
...
// 					break;
				...
				//	Cl_Rinse_Disinf_UpdateTimeInfo();
				//	Cl_Rinse_StartRinse();
...
//	Cl_Rinse_SendRinseStateData();                                                          commented on 28.05.2014
 ...
 Cl_ReturnCodeType Cl_Rinse_CheckforRinseCompletion(void)
 {
	...
	if (hotrinse_flag == 1)
	{
		...
		// ************************************************************************************* added on 131117 to start heater at the begining of open fill
		if (Cl_RinseTotalMinutescounter == 1)
		{
		if (hotrinse_flag ==1)
		{
...
Cl_rinseretcode =  Cl_AlarmActivateAlarms(TEMP2_HIGH_THRESHOLD,false );
Cl_rinseretcode =  Cl_AlarmActivateAlarms(TEMP2_LOW_THRESHOLD,false );
Cl_rinseretcode =  Cl_AlarmActivateAlarms(TEMP1_LOW_THRESHOLD,false );
Cl_rinseretcode =  Cl_AlarmActivateAlarms(TEMP1_HIGH_THRESHOLD,false );
			...
			return CL_ERROR;					// // added on 19.08.2017
		}
// 		if (Cl_RinseTotalMinutescounter >= CL_HOTRINSE_TIMEOUT_MIN )
// 		{
// 			
...
Cl_ReturnCodeType Cl_Rinse_ResetAlertsforReassertion(void )
{
	...
	Cl_RinseAlarmTable[CL_RINSE_ALARM_ACID_OVERRUN].IsRaised = false;
	Cl_AlarmResetAlarm(ACID_OVERRUN);
	Cl_RinseAlarmTable[CL_RINSE_ALARM_ACID_UNDERRUN].IsRaised = false;
	Cl_AlarmResetAlarm(ACID_UNDERRUN);
	Cl_RinseAlarmTable[CL_RINSE_ALARM_BICARB_OVERRUN].IsRaised = false;
	Cl_AlarmResetAlarm(BICARB_OVERRUN);
	Cl_RinseAlarmTable[CL_RINSE_ALARM_BICARB_UNDERRUN].IsRaised = false;
	Cl_AlarmResetAlarm(BICARB_UNDERRUN);
	Cl_RinseAlarmTable[CL_RINSE_ALARM_BP_UNDERRUN].IsRaised = false;
	Cl_AlarmResetAlarm(BP_UNDERRUN);
	Cl_RinseAlarmTable[CL_RINSE_ALARM_BP_OVERRUN].IsRaised = false;
	Cl_AlarmResetAlarm(BP_OVERRUN);
	Cl_RinseAlarmTable[CL_RINSE_ALARM_UFP_OVERRUN].IsRaised = false;
	Cl_AlarmResetAlarm(UFP_OVERRUN);
	Cl_RinseAlarmTable[CL_RINSE_ALARM_UFP_UNDERRUN].IsRaised = false;
	Cl_AlarmResetAlarm(UFP_UNDERRUN);
		Cl_RinseAlarmTable[CL_RINSE_ALARM_CONSOLE_SYNC_LOST].IsRaised = false;
	Cl_AlarmResetAlarm(CONSOLE_SYNC_LOST);
		...
		cl_rinse_powerFailureFlag = 1 ;
			...
			case HOLDER2STATUS_OPEN:
				...
				//	case BC_ALARM:
					...
					case ACID_UNDERRUN:
					case BICARB_OVERRUN:
					case BICARB_UNDERRUN:
					case CONSOLE_SYNC_LOST:
	//		sv_cntrl_setyellowalarm();
...
Cl_ReturnCodeType Cl_Rinse_SendRinseStateData(void)
{
	...
	//Cl_SysStat_GetSensor_Status_Query(SENSOR_TEMP3STATUS,&temp);
		...
		Cl_SysStat_GetSensor_Status_Query(SENSOR_COND_STATUS,&temp);   //uncomment after thermocouple testing
		{
			// manav03 changed
							data.fourbytedata = temp/10;
...
//************************************************************************************  
// 		Cl_SysStat_GetSensor_Status_Query(SENSOR_THERMOCOUPLE_STATUS,&temp);
// 		{
// 			// Gopal: to test Thermo couple 07112017
// 							data.fourbytedata = temp;
// 			 				dataarray[count++] = data.bytedata[0];
// 			 				dataarray[count++] = data.bytedata[1];
// 			 				dataarray[count++] = data.bytedata[2];
// 			 				dataarray[count++] = data.bytedata[3];
...
// 				
		...
		//		sv_prop_startopenfill(); 
				...
				//	Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP1_HIGH_THRESHOLD,0,300,150,0);
					Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP1_LOW_THRESHOLD,0,150,150,0);
					
					Cl_rinseretcode =  Cl_AlarmConfigureAlarmType(TEMP2_LOW_THRESHOLD,0,0,150,0);
...
// 					Cl_rinseretcode =  Cl_AlarmActivateAlarms(FLOW_HIGH_FLOWRATE,true );
			...
			// manav06 commented to be put after openfill
					Cl_rinseretcode =  Cl_AlarmActivateAlarms(TEMP2_HIGH_THRESHOLD,false );
					Cl_rinseretcode =  Cl_AlarmActivateAlarms(TEMP2_LOW_THRESHOLD,false );
					Cl_rinseretcode =  Cl_AlarmActivateAlarms(TEMP1_LOW_THRESHOLD,false );
					Cl_rinseretcode =  Cl_AlarmActivateAlarms(TEMP1_HIGH_THRESHOLD,true );  // enabled to test high inlet temp alarm
					Cl_rinseretcode =  Cl_AlarmActivateAlarms(BP_STOPPED,true );
					Cl_rinseretcode =  Cl_AlarmActivateAlarms(CONSOLE_SYNC_LOST,true );
					Cl_AlarmActivateAlarms( BLD_EVENT,true );
...
Cl_ReturnCodeType Cl_Rinse_DisinfStop(void){
	...
	if(!((cl_rinsestate == CL_RINSE_STATE_IDLE ) || (cl_rinsestate == CL_RINSE_STATE_STOPPED ) ||(cl_rinsestate == CL_RINSE_STATE_CRITICAL_ALARM )  ))
	{
		...
		cl_rinsestate = CL_RINSE_STATE_IDLE;
	//	Cl_rinseretcode = Cl_SendDatatoconsole(CON_TX_COMMAND_DISINF_STOPPED,&data,0);
		Cl_mac_apprequesthandler(EVT_CONSOLE_COMMAND_STOP_DISINF);
		}
}
...
// 						
				...
				//	Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTTEXT,"DOOR",4);
				sv_cntrl_activatevenousclamp();
				IntakeReadyFlag = false;					// Gopal: added since v1 was not turning on after stop n start	
				Cl_Rinsesecondscounter = 0;
				Cl_RinseMinutescounter = 0;
				Cl_RinseTotalMinutescounter = 0;
				Cl_RinsesTotalhourscounter = 0 ;
...
Cl_ReturnCodeType Cl_Rinse_Disinf_UpdateTimeInfo(void)
{
