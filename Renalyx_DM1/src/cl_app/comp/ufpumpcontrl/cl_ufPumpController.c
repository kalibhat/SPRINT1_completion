...
/*int ufhighcounter = 0, ufbcounter = 0 ; */
int uf_rotataion_marker = 0;
			...
			switch (uf_event)
				...
				//	cl_ufpumpFeedback_set_expected_period(uf_final); // not required for now
					...
					cl_ufpumpFeedback_start();
						Cl_AlarmActivateAlarms(UFP_OVERRUN,true);
						Cl_AlarmActivateAlarms(UFP_UNDERRUN,true);
...
// 				}
				...
				Cl_AlarmActivateAlarms(UFP_OVERRUN,false);
				Cl_AlarmActivateAlarms(UFP_UNDERRUN,false);
