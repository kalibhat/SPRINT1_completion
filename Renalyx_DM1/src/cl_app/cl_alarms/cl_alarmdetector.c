...
extern bool highFlowAlarmFlag , lowFlowAlarmFlag , noFlowAlarmFlag ;
...
Cl_Uint8Type flow_alarmid_flag = 0;
...
Cl_ReturnCodeType Cl_LoadAlarmThresholdTable(void)
{
		...
		Cl_alarmThresholdTable.temp1_high_threshold = 370;  /// Gopal: 18.112017 set to lower limit to simulate inlet alarm
		Cl_alarmThresholdTable.temp1_low_threshold = 230;
		...
		Cl_alarmThresholdTable.temp3_low_threshold = 365;   // manav03
		...
		Cl_alarmThresholdTable.tmp_high_threshold  = 1400;
...
Cl_ReturnCodeType Cl_Alarm_translate_sensor_values(void){
	...
	Cl_Uint16Type sensordata = 0 , ps1data  = 0 , ps2data  = 0 , ps3data  = 0 ;
		...
		{
			cond_final = (float)sensordata/10;
		}
...
Cl_ReturnCodeType Cl_alarms_scanforAlarms(void)
{
	...
	if(Cl_alarms_alarms[ACID_OVERRUN].cl_is_enabled)
	{
		
		if (Cl_alarms_alarms[ACID_OVERRUN].cl_alarmstate != CL_ALARM_DETECTED)
		{
			if(cl_sys_statbuffer.acid_fd_back > 100) //Cl_alarms_alarms[TMP_HIGH].cl_upper - 1000)
			{
				...
				Cl_alarms_alarms[ACID_OVERRUN].cl_alarmstate = CL_ALARM_DETECTED;
				Cl_MacEventQueue4.Cl_MacEventData[Cl_MacEventQueue4.Cl_MacNewEventcount][0]=ACID_OVERRUN;
				Cl_alarms_alarms[ACID_OVERRUN].cl_detected_count++;
				cl_alarm_triggered = true;
				cl_lastalarmid = ACID_OVERRUN;
				Cl_MacEventQueue4.Cl_MacEvent[Cl_MacEventQueue4.Cl_MacNewEventcount] = EVT_ALARM_TRIGGERED ;
				Cl_alarms_retcode =  Cl_UpdateMacAlarmEventTable();
				ASSERT_ERROR(Cl_alarms_retcode);
			}
		...
		else if (Cl_alarms_alarms[ACID_OVERRUN].cl_alarmstate == CL_ALARM_DETECTED)
		{
			Cl_alarms_alarms[ACID_OVERRUN].cl_alarmstate = CL_ALARM_INACTIVE;
			cl_alarm_triggered = false;
		}
	}
	
	if(Cl_alarms_alarms[ACID_UNDERRUN].cl_is_enabled)
	{
		
		if (Cl_alarms_alarms[ACID_UNDERRUN].cl_alarmstate != CL_ALARM_DETECTED)
		{
			if(cl_sys_statbuffer.acid_fd_back < 2) //Cl_alarms_alarms[TMP_HIGH].cl_upper - 1000)
			{
				//Cl_Uint16Type temp_ps;
				//temp_ps=ps1;
				...
				//Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTTEXT,"PS1=",4);
				//Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTDATA,&temp_ps,2);
				Cl_alarms_alarms[ACID_UNDERRUN].cl_alarmstate = CL_ALARM_DETECTED;
				Cl_MacEventQueue4.Cl_MacEventData[Cl_MacEventQueue4.Cl_MacNewEventcount][0]=ACID_UNDERRUN;
				Cl_alarms_alarms[ACID_UNDERRUN].cl_detected_count++;
				cl_alarm_triggered = true;
				cl_lastalarmid = ACID_UNDERRUN;
				Cl_MacEventQueue4.Cl_MacEvent[Cl_MacEventQueue4.Cl_MacNewEventcount] = EVT_ALARM_TRIGGERED ;
				Cl_alarms_retcode =  Cl_UpdateMacAlarmEventTable();
				ASSERT_ERROR(Cl_alarms_retcode);
			}
		...
		else if (Cl_alarms_alarms[ACID_UNDERRUN].cl_alarmstate == CL_ALARM_DETECTED)
		{
			Cl_alarms_alarms[ACID_UNDERRUN].cl_alarmstate = CL_ALARM_INACTIVE;
			cl_alarm_triggered = false;
		}
	}
	...
	if(Cl_alarms_alarms[BICARB_UNDERRUN].cl_is_enabled)
	{
		
		if (Cl_alarms_alarms[BICARB_UNDERRUN].cl_alarmstate != CL_ALARM_DETECTED)
		{
			if(cl_sys_statbuffer.bicarb_fd_back < 2) //Cl_alarms_alarms[TMP_HIGH].cl_upper - 1000)
			{
				//Cl_Uint16Type temp_ps;
				//temp_ps=ps1;
				...
				//Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTTEXT,"PS1=",4);
				//Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTDATA,&temp_ps,2);
				Cl_alarms_alarms[BICARB_UNDERRUN].cl_alarmstate = CL_ALARM_DETECTED;
				Cl_MacEventQueue4.Cl_MacEventData[Cl_MacEventQueue4.Cl_MacNewEventcount][0]=BICARB_UNDERRUN;
				Cl_alarms_alarms[BICARB_UNDERRUN].cl_detected_count++;
				cl_alarm_triggered = true;
				cl_lastalarmid = BICARB_UNDERRUN;
				Cl_MacEventQueue4.Cl_MacEvent[Cl_MacEventQueue4.Cl_MacNewEventcount] = EVT_ALARM_TRIGGERED ;
				Cl_alarms_retcode =  Cl_UpdateMacAlarmEventTable();
				ASSERT_ERROR(Cl_alarms_retcode);
			}
		...
		else if (Cl_alarms_alarms[BICARB_UNDERRUN].cl_alarmstate == CL_ALARM_DETECTED)
		{
			Cl_alarms_alarms[BICARB_UNDERRUN].cl_alarmstate = CL_ALARM_INACTIVE;
			cl_alarm_triggered = false;
		}
	}
	
	if(Cl_alarms_alarms[BICARB_OVERRUN].cl_is_enabled)
	{
		
		if (Cl_alarms_alarms[BICARB_OVERRUN].cl_alarmstate != CL_ALARM_DETECTED)
		{
			if(cl_sys_statbuffer.bicarb_fd_back > 100) //Cl_alarms_alarms[TMP_HIGH].cl_upper - 1000)
			{
				//Cl_Uint16Type temp_ps;
				//temp_ps=ps1;
				...
				//Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTTEXT,"PS1=",4);
				//Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTDATA,&temp_ps,2);
				Cl_alarms_alarms[BICARB_OVERRUN].cl_alarmstate = CL_ALARM_DETECTED;
				Cl_MacEventQueue4.Cl_MacEventData[Cl_MacEventQueue4.Cl_MacNewEventcount][0]=BICARB_OVERRUN;
				Cl_alarms_alarms[BICARB_OVERRUN].cl_detected_count++;
				cl_alarm_triggered = true;
				cl_lastalarmid = BICARB_OVERRUN;
				Cl_MacEventQueue4.Cl_MacEvent[Cl_MacEventQueue4.Cl_MacNewEventcount] = EVT_ALARM_TRIGGERED ;
				Cl_alarms_retcode =  Cl_UpdateMacAlarmEventTable();
				ASSERT_ERROR(Cl_alarms_retcode);
			}
		...
		else if (Cl_alarms_alarms[BICARB_OVERRUN].cl_alarmstate == CL_ALARM_DETECTED)
		{
			Cl_alarms_alarms[BICARB_OVERRUN].cl_alarmstate = CL_ALARM_INACTIVE;
			cl_alarm_triggered = false;
		}
	}
...
// 			
		...
		if(Cl_alarms_alarms[FLOW_LOW_FLOWRATE].cl_is_enabled)
		{
			...
			if(lowFlowAlarmFlag)
			{
				...
				lowFlowAlarmFlag = false ;
				
				if(Cl_alarms_alarms[FLOW_LOW_FLOWRATE].cl_alarmstate != CL_ALARM_DETECTED)
				{
					Cl_alarms_alarms[FLOW_LOW_FLOWRATE].cl_alarmstate = CL_ALARM_DETECTED;
					Cl_MacEventQueue4.Cl_MacEventData[Cl_MacEventQueue4.Cl_MacNewEventcount][0]=FLOW_LOW_FLOWRATE;
					Cl_alarms_alarms[FLOW_LOW_FLOWRATE].cl_detected_count++;
					cl_alarm_triggered = true;
					cl_lastalarmid = FLOW_LOW_FLOWRATE;
					Cl_MacEventQueue4.Cl_MacEvent[Cl_MacEventQueue4.Cl_MacNewEventcount] = EVT_ALARM_TRIGGERED ;
					Cl_alarms_retcode =  Cl_UpdateMacAlarmEventTable();
					ASSERT_ERROR(Cl_alarms_retcode);
				}
			...
			else
			{
				if(Cl_alarms_alarms[FLOW_LOW_FLOWRATE].cl_alarmstate == CL_ALARM_DETECTED)
				{
					Cl_alarms_alarms[FLOW_LOW_FLOWRATE].cl_alarmstate = CL_ALARM_INACTIVE;
					Cl_alarms_alarms[FLOW_LOW_FLOWRATE].cl_detected_count = 0;
					cl_alarm_triggered = false;
				}
			}
		...
		if(Cl_alarms_alarms[FLOW_NO_FLOW].cl_is_enabled)
		{
			...
			else
			{
				if(Cl_alarms_alarms[FLOW_NO_FLOW].cl_alarmstate == CL_ALARM_DETECTED)
				{
					...
					Cl_alarms_alarms[FLOW_NO_FLOW].cl_detected_count = 0;
					cl_alarm_triggered = false;
				}
			}
			...
			if(Cl_alarms_alarms[FLOW_HIGH_FLOWRATE].cl_is_enabled)
			{

				//	if(temp1 > Cl_alarmThresholdTable.temp1_high_threshold)

				if(highFlowAlarmFlag)
				{
					//Cl_Uint16Type temp_ps;
					//temp_ps=temp1;
				...
				else
				{
					if(Cl_alarms_alarms[FLOW_HIGH_FLOWRATE].cl_alarmstate == CL_ALARM_DETECTED)
					{
						...
						Cl_alarms_alarms[FLOW_HIGH_FLOWRATE].cl_detected_count = 0;
						cl_alarm_triggered = false;
					}
				}
...
Cl_ReturnCodeType Cl_UpdateMacAlarmEventTable(void)
{
	...
	*/
...

	{
 	...
 	//	Cl_MacEventQueue4.Cl_MacNewEventcountDummy16++;
// 		Cl_MacEventQueue4.Cl_MacNewEventcountDummy8++;
	}
...
//  				else if((cl_alarm_id == FLOW_LOW_FLOWRATE) || (cl_alarm_id == FLOW_NO_FLOW))
//  				{
// 					 Cl_MacEventQueueAlarm.Cl_MacEvent[Cl_MacEventQueueAlarm.Cl_MacNewEventcount] = EVT_ALARM_TRIGGERED ;
// 						flow_alarmid_flag = 1 ;
//  				}
				else 
				{
// 					if(cl_alarm_id == FLOW_LOW_FLOWRATE){
// 						Cl_MacEventQueue4.Cl_MacNewEventcountDummy8++;
// 					}
					flow_alarmid_flag = 0 ;
					Cl_MacEventQueue4.Cl_MacEvent[Cl_MacEventQueue4.Cl_MacNewEventcount] = EVT_ALARM_TRIGGERED ;
